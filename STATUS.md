...
| Onda dourada divisora entre hero e "história" | **PRONTA LOCALMENTE — AGUARDANDO PUSH (terminal)** | A pedido do usuário, inspirado no efeito do "derramamento" (cortina dourada com borda ondulada senoidal), foi criado um divisor `.onda-divisor` entre o hero e a seção história. Diferente do derramamento: **sem pin, sem travar o scroll, sem texto por cima** — decisão explícita do usuário (opção "leve" e "só o efeito visual"). Reaproveita a técnica de `clip-path: polygon()` com seno para a borda ondulada, mas anima via `ScrollTrigger` normal (`scrub`, sem `pin`) conforme o elemento passa pela tela; fade-in na primeira entrada em viewport. Função `poligonoFaixa()` em `landing-sprites-ana-pedro.html`. Fallback `prefers-reduced-motion`: fica estático, sem onda. Arquivo `preview-onda-hero-historia.html` atualizado para carregar GSAP + ScrollTrigger de verdade via CDN (vanilla JS, mesmo código do site real, sem framework) e reagir ao scroll da própria página de preview — não é mais um loop automático simulado |
