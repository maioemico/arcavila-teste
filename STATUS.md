...
>
> Próximo passo: configurar o evento Purchase via Conversions API (Meta) no mesmo cenário — agora desbloqueado tanto pelo pixel novo (ver seção "Redes Sociais — Contas Meta") quanto pelo cenário saudável.

| Item | Status | Observação |
|------|--------|-----------|
| Cenário Make.com | **CORRIGIDO e ATIVO (2026-07-28)** | "Arcavila — Hotmart Compra Aprovada" (ID 5549131). Schema do webhook finalizado e módulo HTTP corrigido (method minúsculo + followAllRedirects). Fila antiga (13 itens vazios/irrecuperáveis) limpa manualmente pelo Chiba. Execução de teste confirmada com sucesso. IDs em `referencia/credenciais-e-ids.md` |
...
| Cenário Arcavila — Hotmart Compra Aprovada | **CORRIGIDO e ATIVO (2026-07-28)** | Webhook recebe Hotmart → HTTP POST Mailchimp API adiciona tag `comprou-amor-e-fe`. Schema do webhook e configuração do módulo HTTP corrigidos, fila antiga limpa, testado com sucesso ponta a ponta. Ver "Pipeline Pós-Compra" para detalhe. ID em `referencia/credenciais-e-ids.md` |
...
| Make.com | Free tier (2 cenários ativos) | Cenário Hotmart → Mailchimp pós-compra corrigido e ativo desde 2026-07-28 (ver "Pipeline Pós-Compra"). O de Publicar Reel (5716956) fica desativado entre publicações desde 2026-07-25 (é on-demand; ativar só na hora de rodar) |
