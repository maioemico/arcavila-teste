...
| amorefe.arcavila.com.br | **NO AR — página nova publicada em 2026-07-07, com retratos reais** | Site imersivo do livro Amor e Fé no estilo da referência abigail-two.vercel.app: preloader de vela, hero pinado, revelações por linha, trilho horizontal de cenas, derramamento de luz até o CTA em creme, canvas em 3 camadas e cursor de luz. Substituiu a landing de vendas anterior (recuperável no git). Commit `11b6077` via GitHub MCP, verificado no navegador. Fonte: `landing-sprites-ana-pedro.html` (build do Cloudflare copia para `anaepedro/index.html`). Retratos `anaepedro/ana.jpg` e `anaepedro/pedro.jpg` publicados via terminal em 2026-07-07 (commit `638e994`). CNAME `amorefe → arcavila-anaepedro.pages.dev` |
...
| Retratos reais de Ana e Pedro (`anaepedro/ana.jpg`, `anaepedro/pedro.jpg`) | **Concluído e no ar (2026-07-07)** | Usuário enviou os retratos reais em 2026-07-07. Convertidos de PNG para JPEG (Ana 1000×1000, 86.799 bytes; Pedro 805×1000, 134.919 bytes) e publicados via terminal local sobre os placeholders quebrados. Commit `638e994`. O arquivo de teste órfão `anaepedro/_test_pedro.jpg` (do diagnóstico do bug de binário via MCP) foi removido no mesmo commit. Confirmado no GitHub: tamanhos batem com os arquivos reais |
...
| Item | Status | Observação |
|------|--------|-----------|
| Imagens dos personagens (Ana e Pedro) | **Concluído em 2026-07-07** | A tentativa via Canva foi abandonada (link expirado, arquivos corrompidos). Usuário enviou os retratos reais diretamente no Cowork; publicados via terminal (commit `638e994`) — ver linha em "Atualizações de Layout" |
...
| Retratos ana.jpg e pedro.jpg no ar | **Concluído em 2026-07-07** | Ver detalhe em "Atualizações de Layout" |
