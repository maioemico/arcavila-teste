...
| amorefe.arcavila.com.br | **NO AR — retratos de Ana e Pedro confirmados ao vivo (base64); última seção removida e publicada; textos de Ana e Pedro reescritos (Laís em destaque), aguardando push terminal** | Site imersivo do livro Amor e Fé no estilo da referência abigail-two.vercel.app: preloader de vela, hero pinado, revelações por linha, trilho horizontal de cenas, derramamento de luz até o CTA em creme, canvas em 3 camadas e cursor de luz. Substituiu a landing de vendas anterior (recuperável no git). Commit `11b6077` via GitHub MCP, verificado no navegador. Fonte: `landing-sprites-ana-pedro.html` (build do Cloudflare copia para `anaepedro/index.html`). Fotos de Ana e Pedro embutidas em base64 (commit `c41dd2b`), confirmadas ao vivo em 2026-07-07 via fetch com anticache. Seção final "fim" (frase "Amor não é o que a gente sente...") removida a pedido do usuário e publicada (commit `a205572`) — a página agora termina no slide "derramamento" (CTA "Ler agora por R$ 37"). Textos descritivos de Ana e Pedro reescritos: mais curtos (máx. 2 linhas), mais impactantes, com Laís (filha do casal) em destaque em ambas as frases, substituindo as palavras "bilhete" e "diesel" rejeitadas pelo usuário; validado por preview antes de aplicar; editado localmente, aguardando push pelo terminal. CNAME `amorefe → arcavila-anaepedro.pages.dev` |
...
| Textos de Ana e Pedro encurtados, Laís (filha) em destaque | **Pronta localmente — AGUARDANDO PUSH (terminal)** | Os dois parágrafos descritivos (`.historia-texto.split-linhas`) sob os títulos de Ana e Pedro foram reescritos: máx. 2 linhas, frases mais impactantes, mantendo o texto de fonte maior (`.historia-titulo`) inalterado. Primeira versão usava "bilhete" e "diesel" — rejeitada pelo usuário, que pediu para destacar a filha **Laís**. Versão final: Ana — "Igreja aos domingos, café pronto, uma vida perfeita ao lado de *Laís*. Até que tudo mudou."; Pedro — "Some de madrugada e volta exausto. Guarda um segredo sobre *Laís* que jurou nunca contar." Validado por preview visual antes de aplicar. Editado em `landing-sprites-ana-pedro.html` |
...
| Textos de Ana e Pedro encurtados, Laís em destaque | **AGUARDANDO PUSH (terminal) — ver "Atualizações de Layout"** | Parágrafos descritivos reescritos, máx. 2 linhas, Laís (filha) em destaque |
