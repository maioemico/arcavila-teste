...
| Cortina dourada em tela cheia entre hero e "história" | **PRONTA LOCALMENTE — AGUARDANDO PUSH (terminal)** | Versão "leve" (banda fina, sem pin) foi rejeitada pelo usuário ("ficou ruim"); substituída pela **réplica fiel do efeito "derramamento"**: nova seção `#transicao` pinada em tela cheia (`.wipe-palco2`), com cortina dourada ondulada (`.wipe-luz2`, reaproveita a função `poligonoOnda()` já existente) crescendo do topo até cobrir a tela, seguida por um painel escuro (`.wipe-creme2`, cor `var(--bg)` para casar com o resto do site, não creme) que sobe revelando a seção história normal logo abaixo. Pin de `+=140%` de scroll extra via GSAP ScrollTrigger, igual em técnica ao derramamento (`+=180%`). Sem texto sobre a cortina (mantida a decisão anterior do usuário). Não duplica o bloco da Ana — a seção história continua intocada, evitando mexer na linha do HTML com a foto em base64. Fallback `prefers-reduced-motion`: cortina e painel ficam ocultos, sem esconder o conteúdo. HTML e JS validados (parser + `node --check`). `preview-onda-hero-historia.html` atualizado para essa nova versão em tela cheia, carregando GSAP + ScrollTrigger reais via CDN |
