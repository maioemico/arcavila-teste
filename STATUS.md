...
| Hero `amorefe.arcavila.com.br` — fonte da frase menor no mobile | **Concluído em 2026-07-07** | A frase "Tem casamento que não acaba em briga. Acaba em silêncio." quebrava em muitas linhas (e até no meio de palavra) no celular, porque o piso do `clamp()` (2.5rem) era grande demais para telas estreitas. Adicionado `@media (max-width:600px){ .hero-frase{font-size:clamp(1.7rem,7.8vw,2.3rem)} }` em `landing-sprites-ana-pedro.html`, reduzindo a fonte só no mobile para caber em até 3 linhas. Commit `5cf33da` via GitHub MCP. Verificação visual no celular pelo usuário ainda pendente |
