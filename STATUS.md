...
| amorefe.arcavila.com.br | **NO AR — retratos de Ana e Pedro em correção (base64), aguardando push terminal** | Site imersivo do livro Amor e Fé no estilo da referência abigail-two.vercel.app: preloader de vela, hero pinado, revelações por linha, trilho horizontal de cenas, derramamento de luz até o CTA em creme, canvas em 3 camadas e cursor de luz. Substituiu a landing de vendas anterior (recuperável no git). Commit `11b6077` via GitHub MCP, verificado no navegador. Fonte: `landing-sprites-ana-pedro.html` (build do Cloudflare copia para `anaepedro/index.html`). Os arquivos `anaepedro/ana.jpg`/`pedro.jpg` foram commitados mas **não apareciam ao vivo** (o build do Cloudflare parece apagar a pasta `anaepedro/` ao gerar o `index.html`) — ver detalhe em "Atualizações de Layout". CNAME `amorefe → arcavila-anaepedro.pages.dev` |
...
| Retratos reais de Ana e Pedro (`anaepedro/ana.jpg`, `anaepedro/pedro.jpg`) | **CORREÇÃO (2026-07-07): não apareciam ao vivo — nova solução aplicada, AGUARDANDO PUSH (terminal)** | Os arquivos `anaepedro/ana.jpg` e `anaepedro/pedro.jpg` foram publicados no GitHub (commit `638e994`) mas a foto não aparecia na página ao vivo — verificado que `amorefe.arcavila.com.br/ana.jpg` e `arcavila-anaepedro.pages.dev/ana.jpg` devolvem o HTML da página, não a imagem. Causa provável: o build do Cloudflare Pages desse projeto apaga a pasta `anaepedro/` ao gerar o `index.html` a partir de `landing-sprites-ana-pedro.html`, levando as imagens junto. Sem acesso ao painel do Cloudflare para corrigir o build. **Solução:** as fotos foram embutidas como base64 direto no `<img src="data:image/jpeg;base64,...">` dentro de `landing-sprites-ana-pedro.html` (editado localmente), o mesmo padrão já usado na capa do catálogo. Isso garante que a imagem viaja junto com o HTML, imune ao build. Detalhe registrado em `referencia/deploy-e-git.md`. Falta o push pelo terminal (arquivo ficou grande, ~334KB, por causa do base64 embutido) |
...
| Item | Status | Observação |
|------|--------|-----------|
| Imagens dos personagens (Ana e Pedro) | **Em correção (2026-07-07)** | A tentativa via Canva foi abandonada (link expirado, arquivos corrompidos). Usuário enviou os retratos reais diretamente no Cowork. Publicados como arquivo `.jpg` separado (commit `638e994`) mas não apareciam ao vivo (build do Cloudflare apaga a pasta). Corrigido embutindo como base64 no HTML — ver linha em "Atualizações de Layout", aguardando push pelo terminal |
...
| Retratos ana.jpg e pedro.jpg no ar | **AGUARDANDO PUSH (terminal) — ver correção em "Atualizações de Layout"** | Base64 embutido no HTML, falta publicar |
