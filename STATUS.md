...
| amorefe.arcavila.com.br | **NO AR — retratos de Ana e Pedro confirmados ao vivo (base64); última seção removida; textos de Ana e Pedro reescritos e publicados (Laís em destaque); texto da seção "a mesma casa" reescrito como cliffhanger dos três personagens, aguardando push terminal** | Site imersivo do livro Amor e Fé no estilo da referência abigail-two.vercel.app: preloader de vela, hero pinado, revelações por linha, trilho horizontal de cenas, derramamento de luz até o CTA em creme, canvas em 3 camadas e cursor de luz. Substituiu a landing de vendas anterior (recuperável no git). Commit `11b6077` via GitHub MCP, verificado no navegador. Fonte: `landing-sprites-ana-pedro.html` (build do Cloudflare copia para `anaepedro/index.html`). Fotos de Ana e Pedro embutidas em base64 (commit `c41dd2b`), confirmadas ao vivo em 2026-07-07 via fetch com anticache. Seção final "fim" (frase "Amor não é o que a gente sente...") removida a pedido do usuário e publicada (commit `a205572`) — a página agora termina no slide "derramamento" (CTA "Ler agora por R$ 37"). Textos descritivos de Ana e Pedro reescritos e publicados (commit `ba808ea`): mais curtos (máx. 2 linhas), mais impactantes, com Laís (filha do casal) em destaque, substituindo as palavras "bilhete" e "diesel" rejeitadas pelo usuário. Título e texto da seção seguinte ("a mesma casa") também reescritos como cliffhanger envolvendo os três personagens; primeira versão soava como se sugerisse um relacionamento entre Pedro e Laís, corrigida deixando explícito o vínculo familiar (pai/mãe/filha); validado por preview e aprovado; editado localmente, aguardando push pelo terminal. CNAME `amorefe → arcavila-anaepedro.pages.dev` |
...
| `amorefe.arcavila.com.br` — pin mobile do slide "derramamento": colapso de altura corrigido | **Corrigido localmente — AGUARDANDO PUSH (terminal)** | O crossfade absoluto entre `.creme-texto` e `.oferta-card` fazia `.creme-conteudo` (pai) colapsar para altura 0 (perdeu conteúdo em fluxo normal ao virar container de filhos `position:absolute`), espremendo texto/oferta numa caixa de 0px — causa real do corte de texto no mobile reportado pelo usuário. Corrigido com `gsap.set(".creme-conteudo", {height: painelAltura, ...})`, `painelAltura = min(82vh, 640px)`, aplicado antes dos filhos virarem absolutos |
| `amorefe.arcavila.com.br` — slide vazio inserido antes do penúltimo slide no mobile (pin "derramamento") | **Editado localmente — AGUARDANDO PUSH (terminal)** | A pedido do usuário, para dar mais fluidez à transição do pin: adicionada uma pausa "vazia" (sem conteúdo visível) entre o fim da cortina de luz/creme e o fade-in do texto (`.creme-texto`), empurrando os tempos de texto/saída/oferta em +0.4 no timeline GSAP e estendendo a duração do pin mobile de `+=320%` para `+=400%`. Dá mais espaço de leitura no slide de texto e transição mais suave para o slide de oferta. JS validado (`node --check`) |
...
| Pin mobile do slide "derramamento" — colapso de altura + slide vazio de transição | **PRONTA LOCALMENTE — AGUARDANDO PUSH (terminal) — ver "Atualizações de Layout"** | Correção do bug que cortava texto no mobile (altura 0 do `.creme-conteudo`) + inserção de uma pausa vazia antes do fade-in do texto, para transição mais fluida do pin |
